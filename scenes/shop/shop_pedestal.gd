...
static var available_common_items:Array[PackedScene] = []

static var available_uncommon_items:Array[PackedScene] = []

static var available_rare_items:Array[PackedScene] = []
...
static var all_uncommon_items:Array[PackedScene] = [preload("res://scenes/items/red_pepper.tscn"), preload("res://scenes/items/onion.tscn"), preload("res://scenes/items/steak.tscn"), preload("res://scenes/items/chicken.tscn")]
static var all_rare_items:Array[PackedScene] = [preload("res://scenes/items/moonshine.tscn")]
static var all_epic_items:Array[PackedScene] = []
static var all_legendary_items:Array[PackedScene] = []
...
func pick_item() -> void:
	...
	# Epic 10%
		...
		# Instantiate a random epic that is not already in the shop
		var item:PackedScene = available_epic_items.pick_random()
		available_epic_items.remove_at(available_epic_items.find(item))
	...
	# Legendary 5%
		...
		# Check if available legendary items are empty
		if len(available_legendary_items) == 0:
			#print("no more legendary")
			pick_item()
			return
		
		# Instantiate a random legendary that is not already in the shop
		var item:PackedScene = available_legendary_items.pick_random()
		available_legendary_items.remove_at(available_legendary_items.find(item))
...
static func reset_item_availability() -> void:
	...
	available_epic_items = all_epic_items.duplicate()
	available_legendary_items = all_legendary_items.duplicate()
